...
### Changed
- **Workflow split.** `status-checks.yml` is replaced by `ci.yml` (lint / test / build / image-scan jobs in parallel, no image push). The Delivery workflow is now narrowly focused: it pushes `:edge` to GHCR on every merge to `master` and on `workflow_dispatch` against `master` (a guard step hard-fails any non-master dispatch with an explanatory error so a stray "Run workflow" click on a PR branch can't masquerade as a successful republish). A new `release.yml` handles versioned releases — on a published GitHub Release it builds, scans, and pushes `:X.Y.Z`, `:X.Y`, `:X`, and `:latest`.
