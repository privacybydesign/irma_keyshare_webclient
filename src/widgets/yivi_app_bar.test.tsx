...
import YiviAppBar from './yivi_app_bar';
...
describe('YiviAppBar language switcher', () => {
  ...
  it('renders an EN and an NL button with the correct disabled/aria-pressed state', () => {
    ...
    const en = screen.getByRole('button', { name: 'EN' }) as HTMLButtonElement;
    const nl = screen.getByRole('button', { name: 'NL' }) as HTMLButtonElement;
  ...
  it('flips i18n language, html lang attribute, and localStorage when the other button is clicked', async () => {
    ...
    expect((screen.getByRole('button', { name: 'NL' }) as HTMLButtonElement).disabled).toBe(true);
    expect((screen.getByRole('button', { name: 'EN' }) as HTMLButtonElement).disabled).toBe(false);
  });
